...
async function main() {
  ...
  const evaluatorSeeds = [
    { fullName: 'Marco Bianchi', email: 'hr@acme.example', role: 'HR' as const, userId: companyAdmin.id },
    { fullName: 'Sara Colombo', email: 'sara.colombo@acme.example', role: 'MANAGER' as const },
    { fullName: 'Elena Ferrari', email: 'elena.ferrari@acme.example', role: 'DIRETTORE_HR' as const },
  ]
  const evaluators = []
