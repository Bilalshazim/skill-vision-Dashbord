...
(function () {
  ...
  // Both modules are now real pages in the separately-built React app
  // (frontend/), served from this same origin at /recruiting and
  // /assessment. Entering a module is therefore a normal top-level
  // navigation there instead of showing the legacy #sv-dashboard screen
  // and loading modules/recruiting.html or modules/assessment.html into
  // an iframe (setModule() below, and the #sv-dashboard screen/iframes/
  // switcher it drove, are left in place but no longer reached from here).
  // sessionStorage.sv_shell_auth (set by handleLogin above) is on this same
  // origin, so the React app's own auth guard sees the same login session.
  function enterDashboard(preferModule) {
    var remembered = store.sget('sv_shell_module');
    var name = preferModule || remembered;
    if (name !== 'recruiting' && name !== 'assessment') name = 'recruiting';
    store.sset('sv_shell_module', name);
